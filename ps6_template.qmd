...
3. 
Yes you can download it using requests. I'm using https://www.geeksforgeeks.org/downloading-files-web-using-python/ as a source to guide me in doing so.
...
base_time_chart=alt.Chart(time_waze_df).mark_circle().encode(
    ...
    rank="rank(count)",
    sort=[alt.SortField("count", order="descending")],
    groupby=["hour"],  # Group by hour to rank data separately for each time
).transform_filter(
    alt.datum.rank <= 10  # Top 10 for each hour
).properties(
